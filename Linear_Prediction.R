...
# PREDICTING HOUSING PRICE WHEN NOX = d7 AND DERIVING STD ERROR + 95% CI
...
# HETEROSKEDASTIC STD ERRORS
# NOTE: USES LM_ROBUST
model3 <- lm_robust(formula = price ~ nox + rooms, data = HPRICE2, se_type = "HC1", alpha = 0.05)
PredPrice=data.frame(nox=c(5), rooms=c(6))
predict(model3, newdata=PredPrice, se.fit=TRUE, interval='confidence')
