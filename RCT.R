...
library(MASS)
library(ggplot2)
library(vtable) #Producing a table in a simple way
library(stargazer)
library(estimatr)
...
W <- runif(bigN,0,5) # Create random numbers each with 0-5 to make the 20% of probability
X=as.integer(W+1)
X1 <- as.numeric(X==1)
X2 <- as.numeric(X==2)
X3 <- as.numeric(X==3)
...
# Generate the Yi(0) and Yi(1) for the numbers
...
# Pull out the random erros from the normal distribution
Sigma <- matrix(c(1,0.75,0.75,1),2,2)
Sigma
e <- (mvrnorm(n=bigN, rep(0, 2), Sigma))

e0 <- e[,c(1)] # vectors with the random draws
mean(e0)
e1 <- e[,c(2)] # vectors with the random draws
mean(e1)
...
# Create a random indicator
D <- as.numeric((runif(bigN,0,1)) > 0.5)

mean(D) # The mean should be around 0.5 because is the random draw from the distribution
...
mA <- lm(formula = X ~ D, data=RCT_DATA) # The real world experiment
...
# When you see the results on the + X. it improves the difference but not a huge difference. 
...
# Undersantond the Summary Statistics table

#The N is the number of observations with your random sampling. It should be 50-50. 
